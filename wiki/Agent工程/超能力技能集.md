...
related: ["[[Claude子Agent机制]]", "[[Claude-Agent团队协作]]", "[[GStack-虚拟工程团队]]", "[[OpenSpec规范]]", "[[Claude-Code游戏工作室实践]]", "[[CLAUDE-md配置方法论]]", "[[Agent技能-addyosmani]]"]
---
...
本质是 [[Claude子Agent机制]] 在工程场景的高密度系统化应用——每个子任务都有独立上下文，避免主 Agent 上下文污染。
...
笨小葱的文章（《AI 增强开发的三件套》）将 Superpowers 与 [[OpenSpec规范]]、[[GStack-虚拟工程团队]] 并称为 AI 增强开发的三件套，分别承担不同职责：

| 工具 | 解决什么 |
|------|---------|
| [[OpenSpec规范]] | "做什么"——规格澄清、变更工件、需求可追溯 |
...
| [[Agent技能-addyosmani]] | Google SWE 全生命周期 20 Skills | ✅ 自动触发 + 手动命令 |
| [[GStack-虚拟工程团队]] | 23 专家角色 sprint 流程 | ❌ 手动 |
| [[Claude-Code游戏工作室实践]] | 49 Agent 层级架构 + Hooks | 部分 |
| [[The-Agency多Agent框架]] | 人格驱动专家库 | ❌ 手动 |

Superpowers 不靠 Agent 数量取胜，靠**自动化流程约束**。单个 Agent + 自动触发 Skills + 子 Agent 派发，比多 Agent 人工协调更流畅。

[[Agent技能-addyosmani]] 与 Superpowers 互补：前者覆盖宽（20 Skills，含 Google SWE Book 实践），后者挖得深（Subagent-Driven Development，强制 TDD 执行）。
