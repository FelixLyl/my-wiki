...
OpenAI 在 Codex 重大更新（[[Codex-全能助手]]）中推出记忆预览版。记忆内容包括：个人偏好、历史修正、费时获取的信息。目标是减少"自定义指令"依赖，让 Agent 在后续任务中自动应用已知上下文。这与 lossless-claw 的树状摘要方案和 Claude-Mem 的 Hooks 捕获方案属同一技术路径的产品化落地。
